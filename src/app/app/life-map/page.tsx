    ...
    <div className="legacy-calm-page space-y-6 pb-12">
      <div className="flex flex-col justify-between gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-card md:flex-row md:items-center">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <Link href="/app/life-map/history" className="flex items-center gap-1.5 rounded-2xl border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 active:scale-95"><History size={15} /> Lịch sử</Link>
          <button onClick={() => void saveDraft()} disabled={saving || draftSaving || !profile} className="rounded-2xl border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 active:scale-95 disabled:opacity-50">{draftSaving ? 'Đang lưu…' : 'Lưu bản nháp'}</button>
          <button onClick={() => void confirmProfile()} disabled={saving || draftSaving || !profile} className="rounded-2xl bg-indigo-600 px-4 py-2 text-xs font-semibold text-white shadow-md shadow-indigo-200 transition hover:bg-indigo-700 active:scale-95 disabled:opacity-50">{saving ? 'Đang lưu…' : 'Xác nhận bản đồ mới'}</button>
        </div>
      </div>
