...
export default function ExperimentDetailPage({ params }: { params: { id: string } }) {
  ...
  const saveReflection = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const json = await response.json(); if (!response.ok) throw new Error(json.error || 'Không thể lưu reflection');
      setMessage('Reflection đã được lưu; learning đang chờ bạn xác nhận.'); await load();
    } catch (err) { setError(err instanceof Error ? err.message : 'Không thể lưu reflection'); }
  ...
  return <div className="legacy-calm-page space-y-6 pb-12"><Link href="/app/experiments" className="inline-flex items-center gap-1.5 text-xs font-semibold text-indigo-600"><ArrowLeft size={14} /> Tất cả thử nghiệm</Link><section className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-card"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><FlaskConical size={22} className="text-indigo-600" /><h1 className="text-xl font-bold text-slate-900">{experiment.title}</h1></div><p className="mt-2 text-sm leading-6 text-slate-600">{experiment.hypothesis}</p></div><span className="rounded-full bg-emerald-100 px-3 py-1 text-[10px] font-bold text-emerald-700">{experiment.status}</span></div><div className="grid gap-3 text-xs text-slate-600 md:grid-cols-3"><p><b>Bước nhỏ:</b> {experiment.smallest_step}</p><p><b>Dấu hiệu thành công:</b> {experiment.success_signal}</p><p><Calendar size={14} className="mr-1 inline" />{experiment.start_date} – {experiment.target_date}</p></div><div><div className="mb-1 flex justify-between text-xs font-semibold"><span>Tiến độ</span><span>{experiment.progress_percent}%</span></div><div className="h-2 rounded-full bg-slate-100"><div className="h-full rounded-full bg-indigo-600" style={{ width: `${experiment.progress_percent}%` }} /></div></div></section>{message && <div className="rounded-2xl bg-emerald-50 p-3 text-xs font-semibold text-emerald-700">{message}</div>}{error && <div className="rounded-2xl bg-rose-50 p-3 text-xs font-semibold text-rose-700">{error}</div>}{experiment.status === 'completed' ? <form onSubmit={saveReflection} className="space-y-4 rounded-3xl border border-indigo-100 bg-indigo-50/50 p-6"><h2 className="text-base font-bold text-indigo-950">Reflection của bạn</h2><textarea required value={form.result} onChange={(event) => setForm({ ...form, result: event.target.value })} placeholder="Điều gì đã xảy ra?" className="min-h-24 w-full rounded-2xl border border-slate-200 bg-white p-3 text-sm" /><textarea required value={form.learningCandidate} onChange={(event) => setForm({ ...form, learningCandidate: event.target.value })} placeholder="Bạn rút ra điều gì?" className="min-h-24 w-full rounded-2xl border border-slate-200 bg-white p-3 text-sm" /><div className="grid gap-3 md:grid-cols-2"><textarea required value={form.feeling} onChange={(event) => setForm({ ...form, feeling: event.target.value })} placeholder="Bạn cảm thấy thế nào?" className="min-h-20 rounded-2xl border border-slate-200 bg-white p-3 text-sm" /><textarea required value={form.nextAction} onChange={(event) => setForm({ ...form, nextAction: event.target.value })} placeholder="Bước tiếp theo?" className="min-h-20 rounded-2xl border border-slate-200 bg-white p-3 text-sm" /></div><label className="block text-xs font-semibold text-slate-600">Mức độ hữu ích: <select value={form.rating} onChange={(event) => setForm({ ...form, rating: Number(event.target.value) })} className="ml-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-normal">{[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value}/5</option>)}</select></label><button disabled={saving} className="flex items-center gap-1.5 rounded-2xl bg-indigo-600 px-5 py-3 text-xs font-semibold text-white disabled:opacity-50"><Save size={14} />{saving ? 'Đang lưu…' : 'Lưu reflection'}</button></form> : <div className="rounded-3xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">Hoàn thành thử nghiệm để mở reflection.</div>}</div>;
}
